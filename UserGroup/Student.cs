...

{
    ...
    int count = 5;
    public static List<double> weights = new List<double>() { 1, 2, 0, 1, 0, 1, 0, 1.5, 1, 0.75, 2, 0, 2 };
    public List<double> Rankings = new List<double>();

    public Student(UserLocation userLocation, RatingResponse ratingResponse)
    {
        LoadInfoFromDB();
        var nearestBibliotheks = LocationFinder.FindNearestLibraries(userLocation.Latitude, userLocation.Longitude, bibliotheks, 5);
        var nearestClinic = LocationFinder.FindNearestClinics(userLocation.Latitude, userLocation.Longitude, clinics, 2);
        var nearestParkings = LocationFinder.FindNearestDisParkings(userLocation.Latitude, userLocation.Longitude, disParks, 1);
        var nearestDoctors = LocationFinder.FindNearestDoctors(userLocation.Latitude, userLocation.Longitude, doctors, 3);
        ...
        var nearestPoliceStations = LocationFinder.FindNearestPoliceStations(userLocation.Latitude, userLocation.Longitude, policeStations, 2);
