...

{
    ...
    int count = 5;
    public static List<double> weights = new List<double>() { 1.5, 1, 0, 1.5, 2, 1, 1.5, 2, 1.5, 1, 2, 2, 0.5 };
    public List<double> Rankings = new List<double>();

    public Family(UserLocation userLocation, RatingResponse ratingResponse)
    {
        LoadInfoFromDB();
        var nearestBibliotheks = LocationFinder.FindNearestLibraries(userLocation.Latitude, userLocation.Longitude, bibliotheks, 5);
        ...
        var nearestKindergartens = LocationFinder.FindNearestKindergartens(userLocation.Latitude, userLocation.Longitude, kindergardens, count);
        var nearestMuseums = LocationFinder.FindNearestMuseums(userLocation.Latitude, userLocation.Longitude, museums, count);
        var nearestMusicSchools = LocationFinder.FindNearestMusicSchools(userLocation.Latitude, userLocation.Longitude, musikSchools, count);
        var nearestParks = LocationFinder.FindNearestParks(userLocation.Latitude, userLocation.Longitude, parks, 8);
        var nearestPoliceStations = LocationFinder.FindNearestPoliceStations(userLocation.Latitude, userLocation.Longitude, policeStations, count);
        var nearestPools = LocationFinder.FindNearestPools(userLocation.Latitude, userLocation.Longitude, pools, count);
        var nearestStops = LocationFinder.FindNearestPublicTransportStops(userLocation.Latitude, userLocation.Longitude, publicTransportStops, count);
        var nearestSchools = LocationFinder.FindNearestSchools(userLocation.Latitude, userLocation.Longitude, schools, count);
        ...
        ratingResponse.SaveRating("fammily", nearestBibliotheks, nearestClinic, nearestParkings, nearestDoctors, nearestKindergartens, nearestMuseums, nearestMusicSchools, nearestParks, nearestPoliceStations, nearestPools, nearestStops, nearestSchools, nearestUniversities);
